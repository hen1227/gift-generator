...
function App() {
    ...
    const UUID = () => {
        // Generate a 64 character (a-z)|(A-Z)|(0-9) string
        let uuid = '';
        for(let i = 0; i < 64; i++) {
            uuid += Math.random().toString(36).substring(2, 3);
        }
        return uuid;
    }
            ...
            {showSidebar && (
                <div className="App-sidebar">
                    <div className={'sidebar-header'}>
                        <div style={{width: '65%'}} className={'new-search'} onClick={()=>{
                            setData(null);
                            setThumbsUpCategories([]);
                            setThumbsDownCategories([]);
                        }}>
                        ...
                        <div style={{width: '15%'}} className={'new-search'} onClick={toggleSidebarVisibility}>
                            <SidebarIcon />
                        </div>
                    </div>
                        ...
                        return (
                            <div key={index} className={'conversation-item'} onClick={()=>{
                                ...
                                <div style={{height: '100%', width: 20}} onClick={()=>{
                                    ...
                                    <TrashCanIcon width={20} fill={'#C9C9D5'}/>
                                </div>
                                {/*<p>{item.uuid}</p>*/}
                            </div>
                        );
